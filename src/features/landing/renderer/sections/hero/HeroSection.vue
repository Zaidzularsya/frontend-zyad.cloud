...
import BrandLogoAnimated from '@/features/branding/components/BrandLogoAnimated.vue'
 ...
 * dibersihkan di sisi client sebagai defense-in-depth via DOMPurify, dengan
 * allowlist tag inline yang sama dengan policy backend.
 */
...
onMounted(() => {
  ...
  // Di canvas builder halaman tidak scroll (iframe), jadi listener parallax
  // hanya menambah beban tanpa efek — lewati.
    ...
    >
      <div class="grid grid-cols-1 items-center gap-gutter lg:grid-cols-12">
        ...
        <div class="fade-up flex flex-col gap-stack-lg lg:col-span-6">
          ...
          <!-- eslint-disable vue/no-v-html -- konten admin tersanitasi backend + DOMPurify di safeTitleHtml -->
          <h1
            v-if="safeTitleHtml"
            class="font-display-xl text-display-xl text-primary"
            aria-label="Hero heading"
            data-field="titleHtml"
          ...
          <p
            class="max-w-xl font-body-lg text-body-lg text-on-surface-variant"
